...
This repo holds the Italian data pack and the pipeline that builds it,
plus [`vocab-engine`](https://github.com/ishmum123/vocab-engine) as a git
submodule at `engine/` (shared UI and drill logic; every language repo
that uses it holds only its own pack). The pack format has no dependency
on the engine, but building the playable site does.
...
tools/
  build_pack.py     reproducible build pipeline (downloads sources, builds pack/*.json)
  check_pack.py     pack-level schema + referential integrity checks
  requirements.txt
...
check.sh            check_pack.py + engine validator + stale-build guard, all in one
...
- **A1** (600 words): every forced item (days, months, seasons, numbers 0-20 plus the
  ...
  `tools/build_pack.py`), then the highest-ranked remaining words.
